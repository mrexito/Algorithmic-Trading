"""Data loading and bootstrap helpers for dashboard callbacks (DB + CSV fallbacks)."""
...
def _write_cached_run(payload: dict[str, object]) -> None:
    """Persist bootstrap metadata so subsequent runs can be skipped when unchanged."""
    try:
...
def _positions_ai(px: pd.Series) -> pd.Series:
    ...
    for i in range(train_period, len(px)):
        try:
            ...
            # On any ML failure, keep previous position and continue.
            pos.iloc[i] = holding
    return pos
